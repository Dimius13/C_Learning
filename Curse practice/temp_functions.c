...
int minT (struct sensor* data, int loadedlines)
{
	...
	for (int i = 0; i < loadedlines; i++)
	{
		// if (data[i].t == -100) printf("month = %d, day = %d, hour = %d, minut = %d, temp = %d \n", data[i].month, data[i].day, data[i].hour, data[i].minuts, data[i].t);
...
// int AverForM (struct sensor* data, int loadedlines, int month)
// {
// 	int sumM= 0, countM = 0;
// 	int averM = 0;
// 	for (int i=0; i < loadedlines; i++)
...
// void grapth (struct sensor* data)
// {
// 	float AvrM [13];
// 	for (int i = 1; i < 13; i++)
// 	{
// 		AvrM[i] = AverForM(data, loadedlines, i);
// 	}
// }